...
to go
  ...
  ;; Each burning tree (red patch) tries to ignite its neighbors.
  ask patches with [ pcolor = red ] [
    ...
    ask neighbors with [ shade-of? pcolor green ] [
      ...
      if burning-neighbors >= burning-neighbors-needed
      [
      set probability probability-of-spread
      ...
      if resultants?[
        ...
        ;; the burning tree is north-west of you
        ;; so the se wind impedes the fire spreading to you
        ;; so increase the probability of spread
        if direction = 45 [
          set probability probability - se-resultant
        ]

        ;; the burning tree is north-east of you
        ;; so the sw wind impedes the fire spreading to you
        ;; so increase the probability of spread
        if direction = 315 [
          set probability probability - sw-resultant
        ]

        ;; the burning tree is south-west of you
        ;; so the sw wind aids the fire spreading to you
        ;; so increase the probability of spread
        if direction = 135 [
          set probability probability + sw-resultant
        ]

        ;; the burning tree is south-east of you
        ;; so the se wind aids the fire spreading to you
        ;; so increase the probability of spread
        if direction = 215 [
          set probability probability + se-resultant
        ]
    ...
    if time-to-burn > 0 [ set time-to-burn time-to-burn - 1]
    if time-to-burn = 0 [set pcolor red - 3.5] ;; once the tree is burned, darken its color
  ]
...
to compute-resultants
  ...
  if south-wind-speed > 0 and west-wind-speed > 0 [set sw-resultant sqrt (south-wind-speed ^ 2 + west-wind-speed ^ 2)]
...
@#$#@#$#@
<experiments>
  <experiment name="Wind-effect" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
